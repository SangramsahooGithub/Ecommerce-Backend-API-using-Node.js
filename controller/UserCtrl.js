...
const UserCtrl={
    ...
    register:async (req,res)=>{
        try{
            ...
            if(mob) return res.status(400).json({msg:"mobile number already exist"})
            const newemail=await User.findOne({email});
            if(newemail) return res.status(400).json({msg:"email already exist"})
            if(password.length < 5)
            return res.status(400).json({msg:"password must be more than 5 charecters "})
    ...
    refreshToken: async (req,res) => {
        try {
            ...
            if(err) return res.status(400).json({ msg: "session expired logi again" });
