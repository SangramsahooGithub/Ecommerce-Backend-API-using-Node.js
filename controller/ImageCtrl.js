...
//for upload a image to cloudinary
//upload img from data in postman
//files is borrowed from express file upload
    upload:async(req,res)=>{
        try{
          ...
          if(file.size > 1*1024*1024 )
          {
              ...
              return res.status(400).json({msg:"file size should be less than 1 mb "})
          }
          if(file.mimetype!=='image/jpeg' && file.mimetype !== 'image/png' && file.mimetype !== 'image/jpg'){
            removeTemp(file.tempFilePath);
            return res.status(400).json({msg:"invalid file format"})
          }
          cloudinary.v2.uploader.upload(file.tempFilePath,{folder:"EcommBackendAPI"},async(err,result)=>{
              if(err) throw err;
              removeTemp(file.tempFilePath);
              res.json({
